...
lives=3;points=0
current_power=None
generate_power=power_var=run=True
start=wincheck=freeze_var=False
FPS=300;clock=py.time.Clock()
...
while run:
    ...
    for event in py.event.get():
            if event.type==py.QUIT:run=False
    win.blit(img_,img_rect)
    win.blit(img2_,img2_rect)
    challenge()
    if start:
        ...
        if asteroids.move:
            ...
            if chosen_power is not None and powers.condition:
                ...
                current_power=d[chosen_power] 
                generate_power=False
            if current_power is not None:
                powers.descend(current_power)
